...
export function narratorDirection(narrator) {
  ...
  return `第一人称角色已确定，整期不可更换：${JSON.stringify(narrator)}。开头只用“这是一段角色化想象”一句说明性质，不说“我不是某某”“借某某的眼睛”或其他跳出角色的制作说明，再以“我是${narrator.name}”进入角色。所有章节正文始终从这个角色的“我”出发讲述，用我的处境、行动或机制解释知识；不得退回第三人称百科介绍，也不得用泛泛的“我来介绍某某”代替角色视角。允许描述其他人或物，但叙述者始终是当前角色。人物经历必须有材料依据，动物或物体拟人化不能改变事实。结束语仍由同一角色说出。写完逐段检查第一人称视角并改写脱离角色的段落，只输出最终讲稿。`;
}
