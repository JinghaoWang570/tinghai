...
test('gender is a hard filter, unknown ages use adult voices, animals may choose all six',()=>{
 ...
 for(const gender of ['male','female'])for(const ageGroup of ['young','adult','senior','unknown']){const n={...narrator,gender,ageGroup};assert.equal(firstPersonVoiceOptions(n).length,3);assert.equal(resolveFirstPersonVoice(n).id,`first-${gender}-${ageGroup==='unknown'?'adult':ageGroup}-v1`);assert.throws(()=>resolveFirstPersonVoice(n,`first-${gender==='male'?'female':'male'}-adult-v1`),/不匹配/)}
...
test('script endpoint rejects wrong-gender or unknown-person voices before model calls; confirmation is signed',async()=>{
 ...
 assert.equal((await call('script',{context,style:'first',voicePreset:'first-female-adult-v1',gender:'female'})).status,400);assert.equal(calls,0);
 ...
 const data=await response.json(),ctx=await unseal(data.context,env.ZHIHU_ACCESS_SECRET,'local-preview');assert.equal(data.voicePreset,'first-female-senior-v1');assert.equal(ctx.narrator.genderBasis,'user_confirmed');assert.ok(data.voices.every(v=>v.gender==='female'));
});
