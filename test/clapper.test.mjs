...
for(const style of ['first','story'])test(`${style} uses described Seed Audio voice and never TTS or podcast`,async()=>{
 ...
 const context=await seal({answer:'回答',script:[{text:'（停顿）这是根据材料作出的角色化讲述。',speaker:'A'}],style,clapperVoice:'温暖女声',references:[]},'test','local-preview');
...
test('invalid clapper script cannot silently become a solo TTS program',async()=>{
 ...
 const context=await seal({query:'袜子',answer:'根据场景选择。',references:[]},env.ZHIHU_ACCESS_SECRET,'local-preview');
