...
const narrator={name:'宋朝书商',kind:'person',gender:'male',genderBasis:'explicit',genderEvidence:'成年男性',ageGroup:'adult',perspective:'我在书市解释印刷术如何改变书籍流通',voiceDescription:'温润的中音，口齿清晰，语速舒缓，带着书商分享见闻的亲切与好奇。'};
...
test('narrator is sealed to context and propagated into the first-person script and audio',async()=>{
 const secret='narrator-test',context=await seal({query:'印刷术',answer:'宋朝书商是一位成年男性',references:[]},secret,'local-preview');let prompts=[];
 ...
 const decoded=await unseal(result.context,secret,'local-preview');assert.equal(decoded.clapperVoice,'成熟男声·沉稳');assert.equal(decoded.voicePreset,'first-male-adult-v1');assert.equal(decoded.narrator.name,narrator.name);
});
